...
const server = createServer((request, response) => {
  ...
  if (request.method === "GET" && request.url === "/api/schedule") {
    ...
    const teams = (settings.teamSnapshot.teams || []).map(team => ({ id: team.id, name: team.name || null, tag: team.tag || null, type: team.type || null, players: (team.members || []).map(uuid => playerNames.get(uuid) || null) }));
