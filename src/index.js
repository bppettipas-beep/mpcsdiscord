...
const server = createServer((request, response) => {
  ...
  if (request.method === "GET" && request.url === "/api/schedule") {
    ...
    const roleByTeam=new Map(Object.values(settings.approvedSignupMessages||{}).filter(record=>record?.status==="approved"&&record.preview?.playerRoles).map(record=>[record.teamId||record.preview.id,record.preview.playerRoles]));
    const teams = sourceTeams.map(team => ({ id: team.id, name: team.name || null, tag: team.tag || null, type: team.type || null, players: (team.members || []).map(member => {const uuid=typeof member==="string"?member:member?.uuid;const supplied=typeof member==="object"&&typeof member?.name==="string"&&!uuidPattern.test(member.name)?member.name:null;return{uuid,name:supplied||team.playerNames?.[uuid]||playerNames.get(uuid)||null,role:team.playerRoles?.[uuid]||roleByTeam.get(team.id)?.[uuid]||null};}) }));
