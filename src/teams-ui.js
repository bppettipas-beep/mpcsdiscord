...
const publicTeamList=s=>{const synced=(s.teamSnapshot?.teams||[]).filter(team=>team?.id),ids=new Set(synced.map(team=>team.id)),previews=Object.values(s.approvedSignupMessages||{}).filter(record=>record?.status==="approved"&&record.preview?.id&&!ids.has(record.preview.id)).map(record=>record.preview);return[...synced,...previews].sort((a,b)=>String(a.name||a.id).localeCompare(String(b.name||b.id)));};
const publicPlayerName=(s,team,uuid)=>team.playerNames?.[uuid]||playerName(s,uuid);
function publicTeamPanel(s,page=0){const teams=publicTeamList(s),pages=Math.max(1,Math.ceil(teams.length/20)),current=Math.max(0,Math.min(pages-1,page)),shown=teams.slice(current*20,current*20+20),components=[];for(let offset=0;offset<shown.length;offset+=5)components.push(new ActionRowBuilder().addComponents(...shown.slice(offset,offset+5).map((team,index)=>new ButtonBuilder().setCustomId(`publicteams:view:${current}:${offset+index}`).setLabel(String(team.name||team.id||"Unnamed Team").slice(0,80)||"Unnamed Team").setStyle(ButtonStyle.Primary))));if(pages>1)components.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`publicteams:page:${current-1}`).setLabel("Previous").setStyle(ButtonStyle.Secondary).setDisabled(current===0),new ButtonBuilder().setCustomId(`publicteams:page:${current+1}`).setLabel("Next").setStyle(ButtonStyle.Secondary).setDisabled(current===pages-1)));return{embeds:[new EmbedBuilder().setColor(0x00e5ff).setTitle("MPCS SIGNED-UP TEAMS").setDescription(teams.length?"Click a team below to see every player on its roster.":"No teams have signed up yet.").setFooter({text:`${teams.length} team${teams.length===1?"":"s"} • Page ${current+1}/${pages}`})],components};}
export async function openPublicTeams(interaction,s){await interaction.deferReply({flags:MessageFlags.Ephemeral});return interaction.editReply(publicTeamPanel(s));}
export async function handlePublicTeams(interaction,s){if(!interaction.isButton()||!interaction.customId.startsWith("publicteams:"))return false;const[,action,value,extra]=interaction.customId.split(":");if(action==="page"){await interaction.update(publicTeamPanel(s,Number(value)||0));return true;}if(action==="back"){await interaction.update(publicTeamPanel(s,Number(value)||0));return true;}const page=Number(value)||0,index=Number(extra),team=publicTeamList(s).slice(page*20,page*20+20)[index];if(!team){await interaction.update(publicTeamPanel(s,page));return true;}const roster=(team.members||[]).map((uuid,index)=>{const discordId=s.links?.[uuid];return `**${index+1}. ${publicPlayerName(s,team,uuid)}**${discordId?` • <@${discordId}>`:""}`;}).join("\n")||"No players listed";await interaction.update({embeds:[new EmbedBuilder().setColor(0x00e5ff).setTitle(String(team.name||team.id||"Unnamed Team").slice(0,256)).setDescription(roster).addFields({name:"Roster Size",value:`${(team.members||[]).length}/8`,inline:true})],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`publicteams:back:${page}`).setLabel("Back to Teams").setStyle(ButtonStyle.Secondary))],allowedMentions:{parse:[]}});return true;}
