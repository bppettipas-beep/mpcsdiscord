...
export function parseSignup(content){
  ...
  let name=null,leaderDiscordId=null;const players=[],labels=new Set(),slots=new Map([["vanilla","Vanilla"],["sword","Sword"],["uhc","UHC"],["cart","Cart"],["spear mace","Spear Mace"],["pot","Pot"],["diamond smp","Diamond SMP"],["sub","Sub"]]);
  for(const line of lines){
    ...
    match=line.match(/^team\s*leader\s*:\s*<@!?(\d{17,20})>\s*$/i);if(match){if(leaderDiscordId)throw new Error("Include exactly one `Team Leader:` line.");leaderDiscordId=match[1];continue;}
    match=line.match(/^([^:]+)\s*:\s*<@!?(\d{17,20})>\s+([A-Za-z0-9_]{3,16})\s*$/i);if(match){const key=match[1].trim().toLowerCase().replace(/\s+/g," "),role=slots.get(key);if(!role)throw new Error(`Unknown roster slot \`${match[1].trim()}\`.`);if(labels.has(role))throw new Error(`Include only one \`${role}:\` line.`);labels.add(role);players.push({discordId:match[2],ign:match[3],role});continue;}
    throw new Error(`I could not read this line: \`${line.slice(0,80)}\``);
  }
  if(!name||!leaderDiscordId)throw new Error("The signup needs `Team name:` and `Team Leader: @user`.");
  if(!/^[A-Za-z0-9 _-]{1,16}$/.test(name)||!teamId(name))throw new Error("Team names must be 1–16 letters, numbers, spaces, `_`, or `-`.");
  if(!players.length)throw new Error("Include at least one gamemode roster line.");const leader=players.find(member=>member.discordId===leaderDiscordId);if(!leader)throw new Error("The `Team Leader` must be one of the players listed in a gamemode or Sub slot.");const roster=[leader,...players.filter(member=>member!==leader)];if(roster.length>8)throw new Error("A team can contain at most eight players.");
...
function signupGuide(){return{embeds:[new EmbedBuilder().setColor(0x00e5ff).setTitle("MPCS TEAM SIGNUP FORMAT").setDescription("Post your team using the exact structure below. The team leader must also be listed in one roster slot.").addFields({name:"Copy this structure",value:"```text\nTeam name: Your Team Name\nTeam Leader: @Discord\n\nVanilla: @Discord ExactMinecraftIGN\nSword: @Discord ExactMinecraftIGN\nUHC: @Discord ExactMinecraftIGN\nCart: @Discord ExactMinecraftIGN\nSpear Mace: @Discord ExactMinecraftIGN\nPot: @Discord ExactMinecraftIGN\nDiamond SMP: @Discord ExactMinecraftIGN\nSub: @Discord ExactMinecraftIGN\n```"},{name:"What you need to do",value:"• Vanilla is the Crystal PvP slot.\n• Use a real Discord mention and exact Minecraft IGN for each roster slot.\n• The `Team Leader` mention must match one listed player.\n• Check every IGN carefully; the bot verifies every Minecraft account.\n• Wait for configured staff to approve the post using ✅."}).setFooter({text:"Do not react to your own signup. Wait for staff approval."})],allowedMentions:{parse:[]}};}
