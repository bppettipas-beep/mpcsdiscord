...
export async function handleSignupApprovalCommand(interaction,settings){
  ...
  if(action==="delete"||action==="wipe"){
    const records=approvalRecords(settings);if(action==="delete"&&!records.length)return interaction.reply({content:"There are no approval-created teams to delete.",flags:MessageFlags.Ephemeral});
    ...
    const token=randomUUID(),teamIds=record?[record.teamId]:[...new Set(records.map(([,value])=>value.teamId))];pendingDestructive.set(token,{userId:interaction.user.id,guildId:interaction.guildId,teamIds,wipe:action==="wipe",expires:Date.now()+120000});
    return interaction.reply({content:action==="wipe"?`⚠️ This will delete **all ${teamIds.length} approval-created teams**, clear their records, and disable signup-format enforcement in the configured channel so people can type normally again. This is confirmation 1 of 2.`:`⚠️ This will delete **${record.preview?.name||record.teamId}**, its approval record, queued create, and affected schedules. This is confirmation 1 of 2.`,components:[confirmationRow(token,1)],flags:MessageFlags.Ephemeral});
  }
...
export async function handleSignupApprovalComponent(interaction,settings){
  ...
  if(value==="1"){await interaction.update({content:`🚨 **FINAL WARNING:** This will permanently remove ${pending.teamIds.length} approval-created team${pending.teamIds.length===1?"":"s"} from the bot and queue their deletion in Minecraft.${pending.wipe?" Signup-channel enforcement will also be disabled.":""} Confirmation 2 of 2.`,components:[confirmationRow(token,2)]});return true;}
  const removed=pending.teamIds.flatMap(id=>removeApprovedSignups(settings,id));if(pending.wipe)delete settings.signupApprovals[pending.guildId];pendingDestructive.delete(token);await settings.save();await interaction.update({content:`✅ Deleted ${removed.length} approval-created team${removed.length===1?"":"s"}${removed.length?`: ${removed.map(id=>`\`${id}\``).join(", ")}`:""}.${pending.wipe?" Signup enforcement is now disabled and the channel accepts normal messages again.":" Minecraft will apply the queued deletion on its next sync."}`,components:[]});return true;
}
...
export async function handleSignupReaction(reaction,user,settings){
  ...
  try{
    ...
    await user.send(`✅ **${signup.name}** was approved and queued for Minecraft with ${profiles.length} player${profiles.length===1?"":"s"}. This confirmation is visible only to you.`).catch(()=>null);return true;
