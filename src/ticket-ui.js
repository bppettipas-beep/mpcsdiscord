...
export async function repairOpenTicketStaffAccess(client,settings){let tickets=0,members=0,failed=0;for(const[key,record]of Object.entries(settings.tickets||{})){const guildId=key.split(":",1)[0],guild=await client.guilds.fetch(guildId).catch(()=>null),channel=guild?await guild.channels.fetch(record.channelId).catch(()=>null):null,config=configFor(settings,guildId);if(!guild||!channel?.isTextBased()||!config)continue;const required=[...new Set([record.pingRoleId,config.supportRoleId].filter(Boolean))].map(id=>guild.roles.cache.get(id)).filter(Boolean);if(!required.length)continue;try{for(const role of required)await channel.permissionOverwrites.edit(role,{ViewChannel:true,SendMessages:true,ReadMessageHistory:true,ManageMessages:true},{reason:"Repair ticket staff access"});const guildMembers=await guild.members.fetch();for(const member of guildMembers.values()){if(member.user.bot)continue;const privileged=member.permissions.has(PermissionFlagsBits.Administrator)||member.permissions.has(PermissionFlagsBits.ManageGuild)||member.permissionsIn(channel).has(PermissionFlagsBits.ManageChannels),ranked=required.some(role=>member.roles.cache.has(role.id)||member.roles.highest.comparePositionTo(role)>0);if(!privileged&&!ranked)continue;await channel.permissionOverwrites.edit(member,{ViewChannel:true,SendMessages:true,ReadMessageHistory:true,ManageMessages:true},{reason:"Repair legacy ticket staff access"});members++;}tickets++;}catch(error){failed++;console.error(`Could not repair staff access for ticket ${record.channelId}:`,error.message);}}return{tickets,members,failed};}
