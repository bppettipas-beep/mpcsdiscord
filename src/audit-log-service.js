...
export class AuditLogService {
  ...
  async memberUpdate(before,after){
    ...
    const automaticJoinRole=this.settings.autoRoles[after.guild.id],added=after.roles.cache.filter(role=>!before.roles.cache.has(role.id)&&role.id!==after.guild.id&&role.id!==automaticJoinRole),removed=before.roles.cache.filter(role=>!after.roles.cache.has(role.id)&&role.id!==after.guild.id);
