...
export class AuditLogService {
  constructor(client,settings,staffGuildId){this.client=client;this.settings=settings;this.staffGuildId=staffGuildId;}
  ...
  enabled(guildId){return !this.staffGuildId||guildId===this.staffGuildId;}
  async send(guild,type,embed){if(!guild||!this.enabled(guild.id))return;const id=this.config(guild.id)[type];if(!id)return;const channel=await this.client.channels.fetch(id).catch(()=>null);if(channel?.isSendable())await channel.send({embeds:[embed.setColor(colors[type]).setTimestamp()]}).catch(error=>console.error(`Could not send ${type} audit log:`,error.message));}
  ...
  async voiceUpdate(before,after){if(!after.guild||(!this.enabled(after.guild.id)))return;for(const [oldValue,newValue,label] of [[before.serverMute,after.serverMute,"Server mute"],[before.serverDeaf,after.serverDeaf,"Server deafen"]])if(oldValue!==newValue){const audit=await this.findAudit(after.guild,AuditLogEvent.MemberUpdate,after.id);await this.send(after.guild,"mod",this.base(`${label} ${newValue?"enabled":"removed"}`,after.member?.user).addFields({name:"Member",value:`<@${after.id}> (${after.id})`},{name:"Moderator",value:audit?this.executor(audit):"Unknown"}));}}
