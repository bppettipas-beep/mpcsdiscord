...
const stages = [
  ...
  { id: "GROUP_STAGE", label: "Group Stage", bestOf: 3 },
...
export async function handleSchedule(interaction, settings) {
  ...
  if (action === "datetime") {
    ...
    settings.schedules.push({ id: `match_${randomUUID().slice(0, 8)}`, stage: stage.id, teamOne: draft.teamOne, teamTwo: draft.teamTwo, bestOf: stage.bestOf, scheduledAt: date.toISOString(), status: "CONFIRMED", createdBy: interaction.user.id, createdAt: new Date().toISOString(), revision: 1 });
