...
export async function checkTeamLeaveDeadlines(settings,client){let changed=false;for(const[key,record]of Object.entries(settings.teamLeaveDeadlines||{})){if(record.deadline>Date.now())continue;const guild=await client.guilds.fetch(record.guildId).catch(()=>null),team=websiteTeams(settings).find(entry=>entry.id===record.teamId);if(!guild||!team){delete settings.teamLeaveDeadlines[key];changed=true;continue;}const members=await guild.members.fetch().catch(()=>null);if(!members)continue;const present=(team.members||[]).filter(uuid=>{const id=settings.links[uuid];return id&&members.has(id);}).length;if(present>=8){delete settings.teamLeaveDeadlines[key];changed=true;continue;}await disbandTeam(settings,client,guild,team,"The team did not restore a full 8-player roster within 24 hours after a member left the Discord server.","automatic-roster-deadline");changed=true;}if(changed)await settings.save();}
